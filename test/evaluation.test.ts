...
test('evaluation separates verifier measurements from unavailable agent baselines', { timeout: 30_000 }, async () => {
  ...
  try {
    ...
    assert.equal(JSON.parse(await readFile(reportPath, 'utf8')).schemaVersion, '1.0.0');
  } finally {
