...
Viewer menampilkan integrity status, jumlah target/element/record, lima behavior contract, lifecycle target, serta 20 evidence record pertama melalui query ber-budget.
...
- Chromium merender empat metric dan lima behavior card tanpa UI error.
...
- Belum ada evidence image gallery, timeline scrubber, provenance graph, atau reference-vs-replica diff.
...
Gate berikutnya: tambahkan visual evidence gallery yang tetap menjaga path confinement, lalu ukur startup/query latency pada package yang lebih besar. MCP sebaiknya memakai service/query core yang sama setelah authorization model ditetapkan.
