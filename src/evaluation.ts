...
type EvaluationCondition = 'screenshot' | 'trace' | 'wbc';
...
function taskEvidenceId(taskId: string, contract: ContractPackage): string | undefined {
  const map: Record<string, string> = {
    'hover-focus-interruption': 'hover-card-enter',
    'css-waapi': 'css-keyframe-pulse',
    'gsap-boolean-numeric-scrub': 'gsap-scrolltrigger-scrub',
  };
  const id = map[taskId];
  return id && contract.behaviors.some((behavior) => behavior.behaviorId === id) ? id : undefined;
}
...
async function buildConditionWorkspace(packageRoot: string, contract: ContractPackage, taskId: string, condition: EvaluationCondition, evidenceByteBudget: number): Promise<{ root: string; evidenceBytes: number }> {
  const root = await mkdtemp(join(tmpdir(), 'wbc-agent-eval-'));
  await cp(resolve(process.cwd(), 'fixtures/replica'), join(root, 'app'), { recursive: true });
  const evidenceRoot = join(root, '.wbc', 'evidence');
  await mkdir(evidenceRoot, { recursive: true });
  const behaviorId = taskEvidenceId(taskId, contract);
  ...
  const evidenceBytes = await directorySize(evidenceRoot);
  if (evidenceBytes > evidenceByteBudget) {
    await rm(root, { recursive: true, force: true });
    throw new Error(`Evidence byte budget exceeded for ${taskId}/${condition}: ${evidenceBytes} > ${evidenceByteBudget}`);
  }
  return { root, evidenceBytes };
}

async function verifyAgentWorkspace(contractPath: string, workspaceRoot: string, taskId: string): Promise<{ status: 'passed' | 'failed' | 'unsupported'; passed: number | null; failed: number | null; error?: string }> {
  const scenarioIds = taskScenarioIds[taskId];
  if (!scenarioIds) return { status: 'unsupported', passed: null, failed: null, error: `No task-specific held-out verifier for ${taskId}` };
  try {
...
export async function evaluatePhase2(
  ...
  outputFile: string,
  repetitions = 3,
  options: { agentCommand?: string; taskId?: string; condition?: EvaluationCondition; timeoutMs?: number; evidenceByteBudget?: number; model?: string; reasoning?: string; seed?: number } = {},
  ...
  if (options.agentCommand) {
    ...
    for (const { task, condition } of ordered) {
      for (let repetition = 1; repetition <= repetitions; repetition += 1) {
        ...
        try {
          const bundle = await buildConditionWorkspace(packageRoot, contract, task.id, condition, evidenceByteBudget);
          ...
          const verifier = run.status === 'completed' ? await verifyAgentWorkspace(resolve(packageRoot, 'behavior-contract.json'), workspace, task.id) : { status: 'not_run' as const, passed: null, failed: null, error: `agent_${run.status}` };
          agentRuns.push({ condition, taskId: task.id, repetition, status: run.status, durationMs: run.durationMs, inputTokens: run.inputTokens, outputTokens: run.outputTokens, evidenceBytes: bundle.evidenceBytes, verifierStatus: verifier.status, verifierPassed: verifier.passed, verifierFailed: verifier.failed, ...(verifier.error ? { errorProvenance: verifier.error } : {}) });
        } catch (error) {
          agentRuns.push({ condition, taskId: task.id, repetition, status: 'failed', durationMs: 0, inputTokens: null, outputTokens: null, evidenceBytes: 0, verifierStatus: 'not_run', verifierPassed: null, verifierFailed: null, errorProvenance: error instanceof Error ? error.message : String(error) });
        } finally {
  ...
  const result: EvaluationReport = {
    schemaVersion: '1.1.0', generatedAt: new Date().toISOString(), packagePath: packageRoot, repetitions,
    ...
    agentHarness: { configured: Boolean(options.agentCommand), commandLabel: options.agentCommand ? 'configured process command' : null, model, reasoning, seed, timeoutMs, evidenceByteBudget, runs: agentRuns, note: options.agentCommand ? 'Runs use isolated starter workspaces and condition-specific evidence bundles; verifier success is separate from process completion.' : 'No agent command configured; A/B/C agent success remains unavailable.' },
  };
